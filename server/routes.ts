...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/health", async (_req, res) => {
    try {
      ...
      console.log(`Health check: Database connection OK (${dbTime}ms)`);
      res.status(200).json({ 
        status: "ok", 
  ...
  app.get("/api/opportunities", async (req, res) => {
    try {
      ...
      console.log(`API request: /api/opportunities - category: ${category}, timeFrame: ${timeFrame}, search: ${search}, limit: ${limit}`);
